...
- 🔐 **Advanced JWT Token Blacklisting**: Protects against session hijacking and malicious token theft. When a user logs out, their JWT is instantly logged in a high-performance server-side blacklist with a precise TTL (Time To Live). This guarantees that even if a token is stolen mid-session, it becomes permanently and instantly revoked the moment the user logs out.
